...
def faz_classificacao(x_treino:pd.DataFrame, y_treino:pd.Series, x_teste:pd.DataFrame, y_teste:pd.Series, min_samples_split:float) -> Tuple[List[float],float]:
    ...
    #RECADO: ao fazer a operação y_teste == y_predicted estava apenas retornando um valor True/False referente as listas/vetore serem iguais ou não, logo fpoi necessário fazer esta operação elemento por elemento no for abaixo
    for i in range(len(y_teste)):
        element = y_teste[i] == y_predicted[i]
        true_false_list.append(element)

    acuracia = np.sum(true_false_list)/len(y_teste)
